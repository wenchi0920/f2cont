...
if ($_GET['step']>1){
	if ($_SESSION['s_dbHost']=="" || $_SESSION['s_dbName']=="" || $_SESSION['s_dbUser']=="" || $_SESSION['s_dbPass']=="" || $_SESSION['s_dbPrefix']==""){ 
		...
		@mysql_query("select no from t3_{$_SESSION['s_dbPrefix']}_ct1 limit 0,1");
		$error=mysql_error();
		if ($error!=""){
			$step_result=false;
			$_GET['step']=2;
...
if ($_GET['step']==3){
	...
	if ($step_result){
		...
		if (strpos($table,";categories;")>0){
			...
			$i=0;
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_ct1 order by no";
			...
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_ct2 order by no";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){			
				$insert_field=array();
				...
				$insert_sql.="INSERT INTO $f2blog_table(id,".implode(",",$insert_field).",isHidden) VALUES('$id','".implode("','",$insert_value)."','0');\r\n";
		...
		if (strpos($table,";logs;")>0){
			...
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){						
				$arr_result['is_public']=($arr_result['is_public']>0)?1:0;			$arr_result['category']=($arr_result['category2']==0)?$arr_result['category1']:$arr_category[$arr_result['category2'].$arr_result['category1']];
		...
		if (strpos($table,";guestbook;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_guest order by no";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$arr_result['parent']=0;
			...
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_guest_reply order by no";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";comments;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_reply order by no";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$arr_result[parent]=0;
		...
		if (strpos($table,";links;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_link order by no";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";attachments;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_files";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";trackbacks;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_trackback";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";dailystatistics;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$query="select * from t3_".$_SESSION['s_dbPrefix']."_count";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$date=$arr_result['set_date'];
				$visitDate=substr($date,0,4)."-".substr($date,4,2)."-".substr($date,6,2);
				$insert_sql.="INSERT INTO $f2blog_table(visitDate,visits) VALUES('$visitDate','".$arr_result['count']."');\r\n";
				//是否分卷
				if(strlen($insert_sql)>=$_SESSION['s_dbSize']*1000){
					$p++;
					$msg=write_file($insert_sql,$filename);  //写入分卷文件
