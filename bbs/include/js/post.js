...
function checklength(theform) {
	...
	var showmessage = postmaxchars != 0 ? '系統限制: ' + postminchars + ' 到 ' + postmaxchars + ' 字節' : '';
	...
	if(in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
		...
		if(special == 3 && isfirstpost) {
			...
			if(theform.vid.value == '') {
				dalert('您還沒有上傳視頻，或者視頻還在上傳中，請稍侯重試。');
				return false;
...
function insertlink(cmd) {
	...
	var str = (tag == 'url' ? '請輸入鏈接的地址:' : '請輸入此鏈接的郵箱地址:') + '<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="txt" />';
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['quote', 'code', 'free', 'hide'])) {
		...
		var ctrlid = editorid + '_cmd_' + cmd;
		var str = '';
		lang['e_quote'] = '請輸入要插入的引用';
		lang['e_code'] = '請輸入要插入的代碼';
		lang['e_free'] = '請輸入要插入的免費信息';
		lang['e_hide'] = '請輸入要插入的隱藏內容';
		if(cmd != 'hide' || !selection) {
			str += lang['e_' + cmd] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5"></textarea>';
		}
		str += cmd == 'hide' ? '<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="txt" checked="checked" />只有當瀏覽者回復本帖時才顯示<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="txt" />只有當瀏覽者積分高於 <input type="text" size="3" id="' + ctrlid + '_param_2" class="txt" /> 時才顯示' : '';
		var div = editorMenu(ctrlid, str);
		...
		if(wysiwyg) {
			...
			var str = '<p>表格行數: <input type="text" id="' + ctrlid + '_param_rows" size="2" value="2" class="txt" /> &nbsp; 表格列數: <input type="text" id="' + ctrlid + '_param_columns" size="2" value="2" class="txt" /></p><p>表格寬度: <input type="text" id="' + ctrlid + '_param_width" size="2" value="" class="txt" /> &nbsp; 背景顏色: <input type="text" id="' + ctrlid + '_param_bgcolor" size="2" class="txt" /></p>';
			var div = editorMenu(ctrlid, str);
...
function insertAttach(id) {
	...
	$('cpdel_' + id).innerHTML = '<a href="###" class="deloption" onclick="delAttach(' + id + ')">刪除</a>';
	$('cpadd_' + id).innerHTML = '<a href="###" title="點擊這裡將本附件插入帖子內容中當前光標的位置"' + 'onclick="insertAttachtext(' + id + ');return false;">插入</a>';
