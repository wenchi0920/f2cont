...
/*
Tool Name: O-blog 2.6 To F2blog
Tool URI: http://korsen.f2blog.com
Description: 转换oblog到F2bLog
Author: korsen
Version: 1.0 适应于F2blog v1.0 & v1.1
请注意oblog的编码比较特殊，生成tt_data.dat文件需要转成utf8编码才能汇入，请参考网上的说明来操作。
*/
...
if ($_GET['step']>1){
	if ($_SESSION['s_dbHost']=="" || $_SESSION['s_dbName']=="" || $_SESSION['s_dbUser']=="" || $_SESSION['s_dbPass']=="" || $_SESSION['s_dbPrefix']==""){ 
		...
		@mysql_query("select id from {$_SESSION['s_dbPrefix']}class limit 0,1");
		$error=mysql_error();
		if ($error!=""){
			$step_result=false;
			$_GET['step']=2;
...
if ($_GET['step']==3){
	...
	if ($step_result){
		...
		if (strpos($table,";categories;")>0){
			...
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."class order by id";
		...
		if (strpos($table,";logs;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."blog";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){					
				$insert_field=array();
				...
				$insert_sql.="INSERT INTO $f2blog_table(".implode(",",$insert_field).",isComment,isTrackback,saveType) VALUES('".implode("','",$insert_value)."','1','1','1');\r\n";
		...
		if (strpos($table,";comments;")>0){
			...
			$i=0;
			$rows=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."remark order by id";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";links;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."link order by id";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
...
<head>
<title>O-blog 以上数据转换为F2blog数据</title>
...
function onclick_step(form,step) {
	...
	if (step==2){
		...
		if (isNull(form.s_dbName, '请输入O-blog数据库名')) return false;
		if (isNull(form.s_dbUser, '请输入O-blog用户名')) return false;
		if (isNull(form.s_dbPass, '请输入O-blog密码')) return false;
...
<?php if  (!isset($_GET['step']) || $_GET['step']<1){?> 
  ...
  <table align="center" class="tablewrap" cellpadding="0" cellspacing="3" width="450">
    <tr>
      <td align="center" class="title">O-blog 2.6以上数据转换为F2blog数据</td>
    </tr>
    <tr>
      <td>
        ...
        <table class="table1" align="center" width="100%">
          <tr>
            <td class="tdrow2">&nbsp;&nbsp;&nbsp;&nbsp;使用之前，请先阅读本说明。<br />
              <br />
&nbsp;&nbsp;&nbsp;&nbsp;该程序只是把 <span class="STYLE1">O-blog 2.6以上</span>中的数据表按照一定的字段对应关系转换成F2blog可以使用的数据。然后通过f2blog.php通用程式汇入到f2blog中，汇入过程中将删除F2blog数据库原有的类别、日志、留言簿、评论、友情链接等数据表的数据，故建议在安装f2blog后马上使用该程式把O-blog数据转换到f2blog中。
<p>&nbsp;&nbsp;&nbsp;&nbsp;该操作将分四步完成。 </p>
            </td>
          </tr>
          <tr>
            <td class="tdrow2">
              <div align="center">
                ...
                <input type="button" name="step_next" value="下一步(O-blog数据库设置)" onclick="onclick_step(this.form,'1')">
              </div>
            </td>
          </tr>
        </table>
...
<?php if  ($_GET['step']==1){?> 
  ...
  <table align="center" class="tablewrap" cellpadding="0" cellspacing="3" width="450">
    <tr>
      <td align="center" class="title">O-blog 数据转成 F2blog 程序</td>
    </tr>
    <tr>
      <td>
        <div class="note">第一步: O-blog 数据库设置 (1/4)</div>
        <table class="table1" align="center" width="100%">
          <tr>
          ...
          <tr>
            ...
            <td class="tdrow2">
              <input type="text" name="s_dbName" class="textbox" value="<?php echo ($_SESSION['s_dbName'])?$_SESSION['s_dbName']:"oblog"?>">
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="tdrow2">
              <input type="text" name="s_dbPrefix" class="textbox" value="<?php echo ($_SESSION['s_dbPrefix'])?$_SESSION['s_dbPrefix']:"ob_"?>">
            </td>
          </tr>  
          <tr>
