	...
	if ($check_info==1){
		if ($mark_id!=""){//编辑
			...
			if ($rsexits!=$mark_id && $rsexits!=""){
				...
				if ($linkImage!=""){
					$sql="update ".$DBPrefix."keywords set keyword='".encode($_POST['name'])."',linkUrl='".$_POST['linkUrl']."',linkImage='$linkImage' where id='$mark_id'";
				}else{
					$sql="update ".$DBPrefix."keywords set keyword='".encode($_POST['name'])."',linkUrl='".$_POST['linkUrl']."' where id='$mark_id'";
				}
				$DMC->query($sql);
				$action="";
			}
			...
			if ($rsexits!=""){
				...
				$sql="INSERT INTO ".$DBPrefix."keywords(keyword,linkUrl,linkImage) VALUES('".encode($_POST['name'])."','".$_POST['linkUrl']."','".$linkImage."')";
				$DMC->query($sql);
				$action="";
			}
		}
