<?php
/*
Plugin Name: randomPosts
Plugin URI: http://joesen.f2blog.com/read-501.html
Description: 随机显示日志
Author: joesen
Version: 1.0
...
function randomPosts_install() {
	$arrPlugin['Name']="randomPosts";
	$arrPlugin['Desc']="随机日志";  
	$arrPlugin['Type']="Side";
	$arrPlugin['Code']="";
	$arrPlugin['Path']="";
...
#随机显示日志
function randomPosts($sidename,$sidetitle,$isInstall){
...
?>
<div id="Side_randomPosts" class="sidepanel">
  ...
  <div class="Pcontent" id="<?php echo "content_$sidename"?>" style="display:<?php echo $display?>">
	...
	$i=0;
	$out_contents="";
	$result = $DMC->query("select a.postTime,b.nickname,a.author,a.id,a.logTitle from {$DBPrefix}logs as a left join {$DBPrefix}members as b on a.author=b.username where a.saveType='1' order by rand() desc Limit 0,$settingInfo[sidelogsPage]");
...
add_filter("randomPosts",'randomPosts',3);
?>