...
if ($_GET['step']>1){
	if ($_SESSION['s_dbHost']=="" || $_SESSION['s_dbName']=="" || $_SESSION['s_dbUser']=="" || $_SESSION['s_dbPass']=="" || $_SESSION['s_dbPrefix']==""){ 
		...
		@mysql_query("select * from {$_SESSION['s_dbPrefix']}comments limit 0,1");
		$error=mysql_error();
		if ($error!=""){
			$step_result=false;
			$_GET['step']=2;
	...
	if ($step_result){
		...
		if (strpos($table,";categories;")>0){
			...
			$arr_fields=array(
					"id"=>"cat_ID",
					"name"=>"cat_name",
					"cateTitle"=>"category_nicename",
					"cateCount"=>"category_count",
			);
			$f2blog_table="f2blog_categories";
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."categories where category_count>0 order by cat_ID";
		...
		if (strpos($table,";logs;")>0){
			...
			$arr_fields=array(
					"id"=>"ID",
					"logTitle"=>"post_title",
					"logContent"=>"post_content",
					"author"=>"post_author",
					"commNums"=>"comments",
					"quoteNums"=>"trackbacks",
					"postTime"=>"post_date",
					"saveType"=>"post_status",
			);
			$f2blog_table="f2blog_logs";
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select distinct * from ".$_SESSION['s_dbPrefix']."posts order by ID";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){	
				$arr_result['post_status']=($arr_result['post_status']=="publish")?1:0;
				if ($arr_result['post_category']==0) $arr_result['post_category']=1;
				...
				$cateresult=mysql_query($catequery); //or die(mysql_error());
				...
				while($tags_result=mysql_fetch_array($tags_query)){
					...
					$arr_tags=mysql_fetch_array($tags_name);
					$tags=(empty($tags))?$arr_tags['tag']:";".$arr_tags['tag'];
				}

				$insert_sql.="INSERT INTO $f2blog_table(cateId,".implode(",",$insert_field).",isComment,isTrackback,tags) VALUES('$cateId','".implode("','",$insert_value)."','1','1','$tags');\r\n";
		...
		if (strpos($table,";comments;")>0){
			...
			$arr_fields=array(
				"id"=>"comment_ID",
				"logId"=>"comment_post_ID",
				"author"=>"comment_author",
				"content"=>"comment_content",
				"ip"=>"comment_author_IP",
				"postTime"=>"comment_date",
				//"homepage"=>"repurl",
				//"email"=>"repemail",
			);
			...
			$i=0;
			$rows=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."comments order by comment_ID";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){				
				$arr_result['comment_date']=strtotime($arr_result['comment_date']); 
		...
		if (strpos($table,";links;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."links order by link_id";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
		...
		if (strpos($table,";tags;")>0){
			...
			$insert_sql.="TRUNCATE TABLE $f2blog_table;\r\n";
			$i=0;
			$query="select * from ".$_SESSION['s_dbPrefix']."tags";
			$result=mysql_query($query); //or die(mysql_error());
			$error=mysql_error();
			while ($arr_result=mysql_fetch_array($result)){
				$insert_field=array();
...
<head>
<title>Wordpress 数据转换为F2blog数据</title>
...
function onclick_step(form,step) {
	...
	if (step==2){
		...
		if (isNull(form.s_dbName, '请输入Wordpress数据库名')) return false;
		if (isNull(form.s_dbUser, '请输入Wordpress用户名')) return false;
		if (isNull(form.s_dbPass, '请输入Wordpress密码')) return false;
...
<?php if (!isset($_GET['step']) || $_GET['step']<1){?> 
  ...
  <table align="center" class="tablewrap" cellpadding="0" cellspacing="3" width="450">
    <tr>
      <td align="center" class="title">Wordpress 数据转换为F2blog数据</td>
    </tr>
    <tr>
      <td>
        ...
        <table class="table1" align="center" width="100%">
          <tr>
            <td class="tdrow2">&nbsp;&nbsp;&nbsp;&nbsp;使用之前，请先阅读本说明。<br />
              <br />
&nbsp;&nbsp;&nbsp;&nbsp;该程序只是把 <span class="STYLE1">Wordpress </span>中的数据表按照一定的字段对应关系转换成F2blog可以使用的数据。然后通过f2blog.php通用程式汇入到f2blog中，汇入过程中将删除F2blog数据库原有的类别、日志、留言簿、评论、友情链接、引用、过滤器、标签、附件、访问日志等数据表的数据，故建议在安装f2blog后马上使用该程式把Wordpress数据转换到f2blog中。
<p>&nbsp;&nbsp;&nbsp;&nbsp;该操作将分四步完成。 </p>
            </td>
          </tr>
          <tr>
            <td class="tdrow2">
              <div align="center">
                ...
                <input type="button" name="step_next" value="下一步(Wordpress数据库设置)" onclick="onclick_step(this.form,'1')">
              </div>
            </td>
          </tr>
        </table>
...
<?php if ($_GET['step']==1){?> 
  ...
  <table align="center" class="tablewrap" cellpadding="0" cellspacing="3" width="450">
    <tr>
      <td align="center" class="title">Wordpress 数据转成 F2blog 程序</td>
    </tr>
    <tr>
      <td>
        <div class="note">第一步: Wordpress 数据库设置 (1/4)</div>
        <table class="table1" align="center" width="100%">
          <tr>
          ...
          <tr>
            ...
            <td class="tdrow2">
              <input type="text" name="s_dbName" class="textbox" value="<?php echo ($_SESSION['s_dbName'])?$_SESSION['s_dbName']:"wordpress"?>">
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="tdrow2">
              <input type="text" name="s_dbPrefix" class="textbox" value="<?php echo ($_SESSION['s_dbPrefix'])?$_SESSION['s_dbPrefix']:"wp_"?>">
            </td>
          </tr>  
          <tr>
      ...
      <td>
        ...
        <table class="table1" align="center" width="100%">
          ...
          <tr>
            ...
            <td width="350" class="tdrow2"><strong>5. 标签</strong></td>
          </tr> 
		  <?php }?>
          <tr>
            <td class="tdrow2" colspan="2">
    ...
    <tr>
      <td>
        ...
        <table class="table1" align="center" width="100%">
          ...
          <tr>
            <td width="100" class="tdrow1"><strong>5. 标签</strong></td>
            <td width="350" class="tdrow2"><?php if (is_int($result_links)){?>“转换了<font color="#FF0000"><?php echo $result_tags?></font> 条”<?php }else{?><font color="#FF0000">“转换失败:<?php echo $result_links?>”</font><?php }?></td>
          </tr>
		  <?php }?>
		  <?php if ($step_result){?>
          <tr>
